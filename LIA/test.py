...
from networks.custom_op import qr_decomposition, diag_embedding
import math
...
class Direction(nn.Module):
    ...
    def forward(self, input):
        ...
        Q = qr_decomposition(weight)  # get eignvector, orthogonal [n1, n2, n3, n4]

        input_diag = diag_embedding(input)  # alpha, diagonal matrix
...
class Synthesis(nn.Module):
    def __init__(self, size, style_dim, motion_dim, blur_kernel=[1, 3, 3, 1], channel_multiplier=1):
        ...
        out_channel = self.channels[8]

        self.conv2 = StyledConv(in_channel, out_channel, 3, style_dim, upsample=True, blur_kernel=blur_kernel)
        self.conv3 = StyledConv(out_channel, out_channel, 3, style_dim, blur_kernel=blur_kernel)

        self.n_latent = self.log_size * 2 - 2

    def forward(self, wa, alpha):
        ...
        out = self.conv2(out, latent[:, 1])
        out = self.conv3(out, latent[:, 2])

        return out


def main():
    net = Synthesis(256, 512, 20)
    net.eval()

    wa = torch.randn(1, 512)
    alpha = [torch.randn(1, 20), torch.randn(1, 20), torch.randn(1, 20)]
