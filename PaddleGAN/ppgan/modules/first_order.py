...
def kp2gaussian(kp_value, spatial_size, kp_variance):
    ...
    coordinate_grid = make_coordinate_grid(spatial_size, kp_value.dtype)
    number_of_leading_dimensions = len(kp_value.shape) - 1
    shape = (1, ) * number_of_leading_dimensions + tuple(coordinate_grid.shape)
    repeats = tuple(kp_value.shape[:number_of_leading_dimensions]) + (1, 1, 1)
    coordinate_grid = coordinate_grid.reshape(shape)
    coordinate_grid = coordinate_grid.tile(repeats)

    # Preprocess kp shape
    shape = tuple(kp_value.shape[:number_of_leading_dimensions]) + (1, 1, 2)
    mean = kp_value.reshape(shape)
